            ...
            <tbody>
              @foreach($reservations as $reservation) 
                <tr>
                  ...
                  <th>{{ $reservation->status_name() }}</th>
                  <td>{{ $reservation->out_date }}</td>
                  <td>{{ $reservation->in_date }}</td>
                  <td><a href="/admin/users/show?id={{ $reservation->user->id }}">{{ $reservation->user->name }}</a></td>
                  <td><a href="/admin/customers/show?id={{ $reservation->project->customer->id }}">{{ $reservation->project->customer->name }}</a></td>
                  <td><a href="/admin/projects/show?id={{ $reservation->project->id }}">{{ $reservation->project->name }}</a></td>
                  <td><a href="/admin/machines/show?id={{ $reservation->rental_machine->machine->id }}">{{ $reservation->rental_machine->machine->name }} / {{ $reservation->rental_machine->machine->type1 }} / {{ $reservation->rental_machine->machine->type2 }}</a></td>
                  <td><a href="/admin/rental_machines/show?id={{ $reservation->rental_machine->id }}">{{ $reservation->rental_machine->code }}</a></td>
                  <td>{{ $reservation->remarks }}</td>
                  <td><a class="btn btn-primary" href="/admin/orders/show?id={{ $reservation->id }}">詳細</a></td>
                </tr>
              @endforeach
            </tbody>
          </table>
